...
export function useResizeDetection(
	...
	const callbackRef = useRef<ResizeCallback | undefined>(callback);
		...
		const handleResize = () => {
			...
			timeoutRef.current = setTimeout(() => {
				...
				if (hasSignificantChange) {
					...
					callbackRef.current?.(newDimensions, prevDimensions);
				}
			}, debounceMs);
		};
		...
		const handleOrientationChange = () => {
			...
			setOrientation(prev => {
				if (newOrientation !== prev) {
					callbackRef.current?.(newOrientation, prev);
